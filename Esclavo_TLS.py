...
def rossler_esclavo(t, state, y_maestro_interp, a, b, c, k):
    x_s, y_s, z_s = state
    y_m = y_maestro_interp(t)
    dxdt = -y_s - z_s
...
def revertir_difusion(difusion, vector_logistico, nmax, ancho, alto):
    ...
    img_reconstruida = (vector_temp * 255).reshape(alto, ancho, 3).astype(np.uint8)
    return Image.fromarray(img_reconstruida)
...
def grafica_serie_temporal(tiempos, x_maestro, y_maestro, z_maestro, t_esclavo, x_esclavo, y_esclavo, z_esclavo):
    # Series temporales en X
    plt.figure(figsize=(10, 6))
    plt.plot(tiempos, x_maestro, label='Maestro x(t)', linewidth = 1.4, color='red') # Señal del maestro
    ...
    print(f"[GRAFICAS] Longitud times (maestro): {len(tiempos)}, Longitud t_esclavo (esclavo): {len(t_esclavo)}")

    # Series temporales en Y
    plt.figure(figsize=(10, 6))
    plt.plot(tiempos, y_maestro, label='Maestro y(t)', linewidth = 1.4, color='red') # Señal del maestro
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(tiempos, z_maestro, label='Maestro z(t)', linewidth = 1.4, color='red') # Señal del maestro
...
def graficar_errores_dispersion(times, x_maestro, y_maestro, z_maestro, t_esclavo, x_esclavo, y_esclavo, z_esclavo, error_x, error_y, error_z):

    # ==================== GRÁFICA ERROR VS TIEMPO X ====================
    plt.figure(figsize=(10, 6))
    plt.plot(times, error_x, linewidth=1.4)
    plt.xlabel("Tiempo")
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(times, error_y, linewidth=1.4)
    plt.xlabel("Tiempo")
    ...
    plt.plot(times, error_z, linewidth=1.4)
    plt.xlabel("Tiempo")
...
def guardar_errores(t_slave, error_x, error_y, error_z):
