...
# ========== RUTAS Y ARCHIVOS ==========
CARPETA_RESULTADOS = Path("Resultados_Solo_Rossler")
...
RUTA_SERIES_VECTORES = CARPETA_RESULTADOS / "series_imagen_rossler.png"
...
def aplicar_confusion(vector_informacion, nmax, rossler_params):    
    ...
    vector_cifrado = vector_informacion + x_cif 
...
def revertir_confusion(vector_cifrado, x_descif, alto, ancho):

    img_recuperada = vector_cifrado - x_descif
    
    img_recuperada = np.round(img_recuperada * 255.0).clip(0, 255).astype(np.uint8)
    img_recuperada = img_recuperada.reshape((alto, ancho, 3))

    return Image.fromarray(img_recuperada)
...
def imagen_rossler(imagen, vector_cifrado, ancho, alto):
    """
...
def graficar_series_vectores(vector_informacion, x_conf):
    plt.figure(figsize=(12, 8))

    # 1. Vector de difusión
    plt.subplot(2, 1, 1)
    plt.plot(vector_informacion)
    plt.ylabel("Difusión")
...
def graficar_dispersion_imagenes(imagen, vector_cifrado):
...
def registrar_tiempos(tiempo_rossler, tiempo_confusion, tiempo_programa, tiempo_descifrado):
    """
...
def main():
    ...
    # 3. Aplicar confusión
    vector_cifrado, tiempo_rossler, tiempo_confusion, t, x_maestro, y_maestro, z_maestro, x_esclavo, y_esclavo, z_esclavo, x_cif, x_descif = aplicar_confusion(vector_inf, nmax, ROSSLER_PARAMS)
    ...
    # Revertir confusion
    imagen_descifrada = revertir_confusion(vector_cifrado, x_descif, alto, ancho)
    ...
    imagen_rossler(imagen, vector_cifrado, ancho, alto)

    # Graficar series de difusion, logistico y rossler
    graficar_series_vectores(vector_inf, x_cif)
    ...
    graficar_dispersion_imagenes(imagen, vector_cifrado)
