...
# ========== RUTAS Y ARCHIVOS ==========
CARPETA_RESULTADOS = Path("Resultados_1883_TLS")
...
def on_connect_keys(client, userdata, flags, rc):
    print(f"[MQTT-ESCLAVO] Conectado (TLS) al broker {BROKER}:{PORT_TLS} (rc={rc})")
    if rc == 0:
        client.subscribe(TOPIC_KEYS, qos=QOS)
        print("[MQTT-ESCLAVO] Suscrito a TOPIC_KEYS (keys) por TLS")
    else:
        print(f"[MQTT-ESCLAVO] Error al conectar (keys/TLS) al broker MQTT (rc={rc})")


def on_connect_data(client, userdata, flags, rc):
    print(f"[MQTT-ESCLAVO] Conectado (sin TLS) al broker {BROKER}:{PORT} (rc={rc})")
    if rc == 0:
        client.subscribe(TOPIC_DATA, qos=QOS)
        print("[MQTT-ESCLAVO] Suscrito a TOPIC_DATA (data) sin TLS")
    else:
        print(f"[MQTT-ESCLAVO] Error al conectar (data/sin TLS) al broker MQTT (rc={rc})")
...
def main():
    ...
    # ---- Cliente TLS para KEYS (PORT_KEYS, 8883) ----
    ...
    client_keys.message_callback_add(TOPIC_KEYS, on_message_keys)
    client_keys.connect(BROKER, PORT_TLS, 60)

    # ---- Cliente sin TLS para DATA (PORT_DATA, 1883) ----
    client_data = mqtt.Client()
    client_data.username_pw_set(USERNAME, PASSWORD)
    client_data.on_connect = on_connect_data
    client_data.message_callback_add(TOPIC_DATA, on_message_data)
    client_data.connect(BROKER, PORT, 60)

    # Arrancar los loops de ambos clientes
    client_keys.loop_start()
    client_data.loop_start()

    print("[MQTT-ESCLAVO] Esperando keys (TLS) y data (sin TLS) del maestro...")
    while RECEIVED_KEYS is None or RECEIVED_DATA is None:
        time.sleep(0.2)

    t_fin_mqtt = time.perf_counter()

    # Detener loops y desconectar
